...
class BinaryFile:
    ...
    def __init__(self, Ly, Lx, filename, n_frames=None, dtype="int16", write=False):
        """
        Open or create a binary file backed by a memory-mapped numpy array
        ...
        PARAMETERS
            ...
            n_frames: int, optional
                Number of frames. Required when writing a new file. Inferred when reading

            dtype: str, default "int16"
                Datatype of each pixel value

            write: bool, default False
                If True, open the file for reading and writing
        ...
        if write and n_frames is None and not os.path.exists(self.filename):
            raise ValueError(
                "Need to provde the number of frames when writing file"
            )
        elif not write:
            n_franes = self.n_frames
    ...
    @staticmethod
    def convert_numpy_file_to_binary(from_filename, to_filename):
        """
        Convert a numpy file (.npy, npz) to a binary file
    ...
    @property
    def shape(self):
        "Returns the dimensions of the data in the file"
        return self.n_frames, self.Ly, self.Lx
    
    @property
    def size(self):
        """Returns the total number of pixels across all frames"""
    ...
    def sampled_mean(self):
        """
        Compute the mean image from up to 1000 evenly spaces frames
    ...
    def write_tiff(self, fname, range_dict={}):
        """
        Write binary file contents to a Tiff file
        ...
        PARAMETERS
            ...
            range_dict: dict, optional
                Dictionary with optional keys "frame_range", "y_range", "x_range",
                each a tuple of (start, stop) indices. Defaults to the full extent 
                of each dimension
        ...
        with TiffWriter(fname, bigtiff=True) as f:
            # Iterate through data to write each frame to tiff
...
class BinaryFileCombined:
    ...
    def __init__(self, LY, LX, Ly, Lx, dy, dx, read_filenames):
        """
        Open multiple binary files for combined reading across ROIs/planes.
